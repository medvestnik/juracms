...
$pdo->prepare('INSERT INTO '.jura_table('pages').' (parent_id,author_id,title,slug,content,status,template,published_at) VALUES (NULL,?,?,?,?,?,?,NOW())')->execute([$uid,'Главная','home','Добро пожаловать','published','home']);
...
$pdo->prepare('INSERT INTO '.jura_table('pages').' (parent_id,author_id,title,slug,content,status,template,published_at) VALUES (NULL,?,?,?,?,?,?,NOW())')->execute([$uid,'Контакты','contacts','Контакты','published','contacts']); $contactsId=(int)$pdo->lastInsertId();
